...
# Van tiết lưu chấm điểm: giới hạn số job ffmpeg+Whisper chạy CÙNG LÚC (chịu tải VPS yếu).
# Spike 100 người nộp → chỉ N chạy, còn lại chờ slot (coroutine chờ, rất nhẹ); clip vẫn "queued"
# nên client poll thấy chưa xong → hiện "mạng chậm", KHÔNG làm sập server.
_score_sem = asyncio.Semaphore(max(1, settings.scoring_concurrency))
...
async def run_scoring(clip_id: str, user_id: str, duration: float, lesson_xp: int) -> None:
    ...
    async with _score_sem:  # xếp hàng khi quá tải, không spawn vô hạn ffmpeg/Whisper
